...
class User(PaginatedAPIMixin, UserMixin, db.Model):
    ...
    message_sent: so.WriteOnlyMapped["Messages"] = so.relationship(
        "Messages", foreign_keys="Messages.sender_id", back_populates="author")
    message_received: so.WriteOnlyMapped["Messages"] = so.relationship(
    ...
    def unread_message_count(self):
        ...
        query = sa.select(Messages).where(Messages.recipient == self, Messages.timestamp > last_read_time)
...
class Messages(db.Model):
    ...
    author: so.Mapped[User] = so.relationship("User", foreign_keys="Messages.sender_id", back_populates="message_sent")
    recipient: so.Mapped[User] = so.relationship("User", foreign_keys="Messages.recipient_id", back_populates="message_received")
